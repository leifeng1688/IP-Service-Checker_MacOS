...
# ── 跨平台打包相容處理 ───────────────────────────
# PyInstaller (Windows): sys._MEIPASS
# py2app (macOS):        sys.frozen == 'macosx_app'
...
class API:
    ...
    def run_scan(self, params):
        ...
        def do_scan():
            try:
                if mode == "tcp":
                    ips   = [l.strip() for l in params.get("hosts","").splitlines() if l.strip()]
                    ports = [int(p.strip()) for p in params.get("ports","").replace(","," ").split() if p.strip().isdigit()]
                    ...
                    with ThreadPoolExecutor(max_workers=threads) as ex:
                        ...
                        for f in as_completed(futs):
                            ...
                            send(f"{r['ip']}:{r['port']} → {r.get('status','')} | {r.get('service','')}",
                                 "open" if r.get("status")=="OPEN" else "closed")
                    finish(results)

                elif mode == "http":
                    urls = [l.strip() for l in params.get("hosts","").splitlines() if l.strip()]
                    results = []
                    with ThreadPoolExecutor(max_workers=threads) as ex:
                        ...
                        for f in as_completed(futs):
                            ...
                            send(f"{r['url']} → {code} | {r.get('server','')}",
                                 "open" if str(code).startswith("2") else "closed")
                    finish(results)

                elif mode == "ssl":
                    hosts = [l.strip() for l in params.get("hosts","").splitlines() if l.strip()]
                    ...
                    with ThreadPoolExecutor(max_workers=threads) as ex:
                        ...
                        for f in as_completed(futs):
                            ...
                            send(f"{r['host']}:{r['port']} → CN:{r.get('tls_cn','—')} | 到期:{r.get('tls_not_after','')} ({days}天)", t)
                    finish(results)
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  textarea, input[type=text], input[type=number] {
    ...
    padding:8px 10px; outline:none; resize:vertical; transition:border-color .15s;
  }
...
<div class="tabs">
  <div class="tab active" id="tab-tcp">TCP</div>
  <div class="tab" id="tab-http">HTTP</div>
  <div class="tab" id="tab-ssl">SSL</div>
</div>
<div class="main">
  <div class="panel-left">
    <div class="panel-body">
      ...
      <button class="btn btn-primary" id="btnRun">▶ RUN SCAN</button>
      <div class="btn-row" style="margin-top:16px">
        <button class="btn btn-ghost" id="btnHtml" disabled>↗ HTML</button>
        <button class="btn btn-ghost" id="btnCsv"  disabled>↓ CSV</button>
      </div>
      <button class="btn btn-ghost" id="btnBrowser"
...
<div class="bottom-bar">
  <div class="view-toggle">
    <button class="vtab active" id="vLog">LOG</button>
    <button class="vtab"        id="vTable">TABLE</button>
  </div>
...
// 用 addEventListener 取代 inline onclick，相容 WebView2
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('tab-tcp').addEventListener('click', function(){ switchMode('tcp'); });
  document.getElementById('tab-http').addEventListener('click', function(){ switchMode('http'); });
  document.getElementById('tab-ssl').addEventListener('click', function(){ switchMode('ssl'); });
  ...
  document.getElementById('btnBrowser').addEventListener('click', openBrowser);
  document.getElementById('vLog').addEventListener('click', function(){ switchView('log'); });
  document.getElementById('vTable').addEventListener('click', function(){ switchView('table'); });
});
...
function renderResults(payload, mode) {
  ...
  if (mode==='tcp') {
    ...
    rows=results.map(r=>`<tr>
      ...
      <td>${r.service||''}</td><td class="muted">${(r.banner||'').slice(0,60)}</td>
      <td>${r.tls_cn||''}</td><td>${r.tls_issuer||''}</td><td>${r.tls_not_after||''}</td>
      <td class="${dayClass(r.tls_days_left)}">${r.tls_days_left!=null&&r.tls_days_left!==''?r.tls_days_left+'d':''}</td>
    ...
    rows=results.map(r=>`<tr>
      ...
      <td>${r.tls_cn||''}</td><td class="muted">${r.tls_san||''}</td>
      <td>${r.tls_issuer||''}</td><td>${r.tls_not_before||''}</td><td>${r.tls_not_after||''}</td>
      <td class="${dayClass(r.tls_days_left)}">${r.tls_days_left!=null&&r.tls_days_left!==''?r.tls_days_left+'d':''}</td>
      <td class="muted">${r.tls_cipher||''}</td>
...
function runScan() {
  ...
  ['btnHtml','btnCsv','btnBrowser'].forEach(id=>document.getElementById(id).disabled=true);
...
if __name__ == "__main__":
    ...
    if sys.platform == "win32":
        # private_mode=False 讓 WebView2 可以快取，加速後續啟動
        # storage_path 指定快取位置避免每次重新初始化
        import tempfile
        cache_dir = os.path.join(tempfile.gettempdir(), "IPServiceChecker_webview")
